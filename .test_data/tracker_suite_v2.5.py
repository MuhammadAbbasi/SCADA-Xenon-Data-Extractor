...
from matplotlib.backends.backend_pdf import PdfPages # KEY FOR PDF EXPORT
...
def worker_plot_file(args):
    # For batch PNG generation (Optional Step 4)
    import matplotlib.pyplot as plt
    matplotlib.use('Agg') 
...
def worker_generate_pdf(csv_path, overview_dir, output_pdf, date_str):
    ...
    try:
        ...
        with PdfPages(output_pdf) as pdf:
            ...
            # --- PART 1: ADD EXISTING OVERVIEW PLOTS ---
            # We look for files generated in Step 3
            ...
            # Filter to ensure we only get the Overview ones (contain 'ALL' or 'NCU'), not individual 'TX_' ones
            overview_files = [f for f in overview_files if "TX_" not in os.path.basename(f) and "each_tracker" not in f]
            
            if not overview_files:
                print("Warning: No overview PNGs found. Did you run Step 3?")
            
            for img_path in overview_files:
                try:
                    img = mpimg.imread(img_path)
                    # Create a figure to hold the image
                    fig_img, ax_img = plt.subplots(figsize=(11.69, 8.27)) # A4 Landscape roughly
                    ax_img.imshow(img)
                    ax_img.axis('off')
                    ax_img.set_title(f"Overview: {os.path.basename(img_path)}", fontsize=10)
                    pdf.savefig(fig_img, orientation='landscape')
                    plt.close(fig_img)
                except Exception as ex:
                    print(f"Skipped corrupt image {img_path}: {ex}")

            # --- PART 2: GENERATE INDIVIDUAL PLOTS FROM DATA ---
            ...
            # Group by Tracker
            grouped = df.groupby(["NCU", "TCU"])
            
            count = 0
            for (ncu, tcu), group in grouped:
                count += 1
                ...
                if not target.empty: 
                    ax.plot(target["Timestamp"], target["Valore"], label="Target", color="blue", lw=1)
                if not actual.empty: 
                    ax.plot(actual["Timestamp"], actual["Valore"], label="Actual", color="red", lw=1)
        ...
        return f"Critical Error generating PDF: {str(e)}"
...
class TrackerSuiteApp:
    def __init__(self, root):
        self.root = root
        self.root.title("SCADA Tracker Suite v2.5 (PDF Report)")
    ...
    def setup_layout(self):
        ...
        ttk.Entry(cfg, textvariable=self.folder_path, width=45).pack(side="left", padx=5)
        ttk.Button(cfg, text="Browse", command=self.browse).pack(side="left")
        ttk.Label(cfg, text="Date:").pack(side="left", padx=(20, 5))
        self.cal = DateEntry(cfg, width=12, background='darkblue', foreground='white', borderwidth=2, date_pattern='yyyy-mm-dd', textvariable=self.date_val)
        self.cal.pack(side="left", padx=5); self.cal.bind("<<DateEntrySelected>>", self.trigger_check)
        ttk.Button(cfg, text="Refresh", command=self.check_status).pack(side="right")
        ...
        self.btn_step4 = tk.Button(split, text="4. GENERATE INDIVIDUAL PLOTS\n(Optional Batch)", bg="#ddd", state="disabled", command=self.run_step4_indiv)
    ...
    def check_status(self):
        ...
        if os.path.exists(p_merged):
            ...
            self.btn_pdf.config(state="normal", bg="#CCBDE7") # Unlock PDF Button
        else:
            ...
            self.btn_pdf.config(state="disabled", bg="#ddd") # Lock PDF
    ...
    def exec_indiv(self):
        print("Batch Plotting..."); base, date = self.folder_path.get(), self.date_val.get()
        ...
        print("Batch Done."); self.root.after(0, self.check_status)
    ...
    def exec_pdf(self):
        ...
        # 1. Ask where to save
        default_name = f"Tracker_Report_{date}.pdf"
        out_pdf = filedialog.asksaveasfilename(initialfile=default_name, defaultextension=".pdf", filetypes=[("PDF Files", "*.pdf")])
        ...
        # Call worker directly (no multiprocessing for PDF generation to avoid complexity)
    ...
    def on_double_click_plot(self, event):
        ...
        pass # Placeholder for brevity, paste your double click logic here
