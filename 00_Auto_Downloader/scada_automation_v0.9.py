...
def process_hourly_report(target_date, target_hour):
    ...
    if os.path.exists(check_path):
        if validate_existing_csv(check_path, target_date, target_hour):
            print(f"  ✓ Tracker file already exists and is valid for {target_date} {target_hour:02d}:00 ({filename}). Skipping!")
            current_status = f"File già presente e corretto: {filename}"
            return True
        else:
            print(f"  ⚠ Existing file '{filename}' is incorrect or corrupted. Renaming to _incorrect and re-downloading...")
